...
Features:
	1.  mapping URL to handler_function by decorator:
			@get('/:id')
			def index(id):
				return '<h1>Hello %s!</h1>' % id
		This feature can help testing the func without turning on the server
	...
	2. 	intercept by URL:
		...
		the interceptor accept a next() func and decides whether return Error or do next()

	3. 	create view with template:
			@view('index.html')
			@get('/)
			def index():
				return dict(content='<h1>hello</h1>')
...
_TIMEDELTA_ZERO = datetime.timedelta(days=0,hours=0,seconds=0,minutes=0,microseconds=0)
...
_re_route = re.compile(r'(\:[a-zA-Z_]\w*)')		# match '\:_varname01'
...
class Route(object):
	...
	def __init__(self, func):
		self.path = func.__web_route__ # self.path effect Rounte only at __init__
		self.method = func.__web_method__
		self.is_static = _re_route.search(self.path) is None # no variable in path
		if not self.is_static:
			# route is just a _RE_
...
def favicon_handler():
	return static_file_generator('/favicon.ico')
...
class Request(object):
	...
	def _parse_input(self):
		'''
		get input from 'wsgi.input'
		once parsed, the data would vanish

		>>> r = Request({'REQUEST_METHOD':'POST', 'wsgi.input':StringIO('a=1&b=M%20M&c=ABC&c=XYZ&e=')})
		>>> r._parse_input()
		{'a': u'1', 'c': [u'ABC', u'XYZ'], 'b': u'M M', 'e': u''}
		>>> r._parse_input()
		{}
	...
	def __getitem__(self,key):
		'''
		Get input parameter value. If the sepecified key has multiple value, the first one is returned.
	...
	def gets(self, key):
		...
		Returned value will always be a list.
	...
	def environ(self):
		...
		Return a dict which can be referred as x.y(class Dict)
	...
	def header(self, header, default=None):
		'''
		Get header from request as unicode, return None if not exist, or default if sepecified
	...
	def cookie(self, name, default=None):
		''' 
		Return sepecified cookie value as unicode. Default to None if cookie not exists.
...
class Response(object):
	...
	def header(self, name):
		...
		if not key in _RESPONSE_HEADER_DICT:
			# if not in _RESPONSE_HEADER_DICT, stay what they are(case)
			key = name
	...
	def set_cookie(self,name,value,max_age=None,expires=None,path='/',domain=None,secure=False,http_only=True):
		...
		Args:
			...
			http_only: if the cookie is for http only, default to True for better safty
						(client-side script connot access cookies with HttpOnly flag).
...
def _build_pattern_fn(pattern):
	'''
	pattern = '*/inter/' would match any url ends with 'inter/'
	pattern = '*/inter/' would match any url ends with 'inter/'
	pattern = '/inter/' would match any url starts with '/inter/'
	'''
...
def _build_interceptor_fn(func, next):
	'''
	Return a new fun func_new, which call func first and then call next
	The funcs in middle should return next()  and the last has no parameter.
	'''
	def _wrapper():
		if func.__interceptor__(ctx.request.path_info):
		# interceptor check passed, do what should be done
			return func(next)
		else:
...
def _build_interceptor_chain(last_fn, *interceptors):
	...
	Final func doesn't decorate with interceptor.
...
def _load_module(module_name):
	...
	if last_dot == (-1):
	# (-1) indecates that there is no DOT in the string
...
class WSGIApplication(object):
	...
	def add_module(self, mod):
		...
		# >>> md = _load_module('time')
		# >>> md.ctime()
		# 'Sat Mar 26 00:44:10 2016'
		# >>> import time
		# >>> time == md
		# True
